...
**Workflow:**
1. **Select Engine**: Choose database engine (MySQL, PostgreSQL, MongoDB, Redis, SQLite)
...
- Password stored securely in systemd service
...
If scheduled backups aren't running, fix timer dependencies:
...
- **Compression**: Supported (`.gz`)

### Redis
- **Authentication**: Not required
- **Backup Format**: RDB file (`.rdb`)
- **Compression**: Supported (`.rdb.gz`)

### SQLite
- **Authentication**: Not required
- **Backup Format**: Database file (`.db`) or SQL dump (`.sql`)
- **Compression**: Supported (`.db.gz`, `.sql.gz`)
